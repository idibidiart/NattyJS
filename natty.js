 ...
 * its inner/outer HTML) to be replaced, in global, dynamic fashion, with JSON 
 * data within the inner/outer HTML scope of a newly created instance of the Node
 * Prototype. 
 *
 ...
 *
 * Natty works by populating the special variables in a newly created instance 
 * of the Node Prototype with dynamic JSON data (where the JSON keys must 
 ...
 * from which instances can be created, populated with JSON data and then 
 * inserted into the Node (in append, prepend, and replace modes, with the 
 * ability to target specific instances of the Node Prototype or the Node's 
 * entire content) where the Node itself can be dynamically linked into other 
 *  Nodes and so on...
 * 
 * Examples: 
 *
